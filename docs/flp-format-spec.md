...
| `0xDB` | DATA (varint + bytes) | Channel Levels struct | 24-byte fixed record | the channel levels event on FL 25 (Python's the reference parser points to 0xCB but 0xCB is taken by Name; FL 25 emits Levels at 0xDB). One per channel. Fields: `int32 pan`, `uint32 volume`, `int32 pitch_shift`, `uint32 filter_mod_x`, `uint32 filter_mod_y`, `uint32 filter_type`. Default values `{pan: 6400, volume: 10000, pitch_shift: 0, filter_mod_x: 256, filter_mod_y: 0, filter_type: 0}` — Python's flp-info exposes these as normalized floats `volume/12800 = 0.78125` and `pan/6400 = 1.0`. |
